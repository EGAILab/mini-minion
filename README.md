...
**Concurrent file writes (MEM-GAP-009).** Every background writer that can touch an agent's memory files (an interactive turn, `CaptureWorker`, the daily schedulers, ...) is a thread inside the same live bot process. `MemoryFileRepository` serializes every write method (`remember`, `remember_import`, `write_digest`, `append_daily`, `delete`, `delete_import`) with a per-target-path `threading.Lock` (`_lock_for()`), so two threads writing the same file can never interleave or lose an update — most importantly for `append_daily()`, whose read-modify-write would otherwise let one writer's read miss another's concurrent write. Scoped per-instance (per-agent), not global, since two agents' repositories never share a file.

**Cross-process locking for `append_daily` (R3-GAP-010).** The `threading.Lock` above only serializes threads *inside one process* — a genuinely separate process (the standalone `minion-assist memory ...` CLI, which can run while the bot is live) has its own, entirely unrelated lock dict, so it was never actually protected against. For every write method except `append_daily`, that's harmless: a blind overwrite/delete racing another blind overwrite/delete just picks whichever wrote last — ordinary filesystem semantics, not a lost update — and `MemoryConsolidator.approve()` additionally already has its own content-hash pre-image check (`StaleProposalError`), a *better* guard than a lock since it refuses a stale write outright rather than silently clobbering it. `append_daily()` is the one genuine read-modify-write, so it now also holds `_cross_process_lock()` — a dependency-free OS-level lock, `os.O_CREAT | os.O_EXCL` atomically creating a sidecar `.{filename}.lock` file (identical semantics on POSIX and Windows) — for the same critical section its `threading.Lock` already covers. A lock file older than 30 seconds is treated as abandoned by a crashed holder and forcibly removed, so a crash can't deadlock every future writer; a genuinely contested lock waits up to 10 seconds before raising `CrossProcessLockTimeout` rather than hanging forever. `_atomic_write_text`'s existing temp-file-then-rename swap already guaranteed a crash leaves either the old or the complete new file, never partial content — no new work needed there.
...
**A single, direct outage signal (R3-GAP-008).** Every DB-backed path already degraded safely and recorded its own failure independently on an outage (per-worker `WorkerHealth`, `session_writes:{agent}`, a per-agent `queue_lag_summary` try/except) — but that meant an operator watching `/status deep` during a live PostgreSQL outage had to *notice* several scattered lines going bad at once and infer a shared cause, rather than seeing one clear signal. `SessionDB.ping()` is a cheap, direct `SELECT 1` probe (never raises — returns `None` when reachable, or a short `"{ExceptionType}: {message}"` description) that both `/status deep` and the standalone `minion-assist memory status --deep` CLI now print as a single `Database: reachable` / `Database: UNREACHABLE — ...` line before anything else, so an outage is visible at a glance rather than by correlation. This is purely observational — recovery itself needed no new mechanism: mirror gaps already self-heal via `ReconciliationScheduler`'s periodic, fully exhaustive `reconcile_all_sessions` pass, and job-lane gaps already self-heal via the same scheduler (recent) plus `backfill_agent`/`embed_backfill_agent` (exhaustive, R3-GAP-003 above) for anything older.
...
The test suite covers **3336 cases** across all modules. Three are skipped by default: one (`tests/test_providers_base.py`) requires the `anthropic` package; one (`tests/test_mcp_schema.py`) is a Windows-specific test skipped when its own environment precondition isn't met; one requires a live PostgreSQL instance (see below). The Matrix channel tests in `tests/matrix/` pass without any Matrix server or matrix-nio installation.
