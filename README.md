...
| `session_job_coverage` | One row per `(session_id, lane)` (`lane` in `capture`/`commitment`/`message_embedding`) — the durable "highest id a job has ever been enqueued for" cursor reconciliation reads instead of the job tables themselves, so retention pruning old job rows can't reset it. Columns: `agent_id`, `session_id`, `lane`, `covered_through_id`, `channel` (commitment lane only), `updated_at`. |

Completed/failed rows in the three job-queue tables above (`message_embedding_jobs`, `memory_capture_jobs`, `memory_commitment_jobs`) accumulate indefinitely unless `memory_retention` is configured — see "Operational-table retention" (MEM-GAP-015). `memory_proposals`/`commitments` survive that cleanup (pruning a job only detaches its now-nullable back-pointer, never the output itself — `ON DELETE SET NULL`, not `CASCADE`); `session_job_coverage` is never touched by it at all.
