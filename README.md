...
Every cross-table reference within `SessionDB`'s own tables is now a real foreign key with `ON DELETE CASCADE`, not just a plain unconstrained column — `messages`/`message_mirrors`/`message_embeddings`/`message_embedding_jobs`/`memory_capture_jobs`/`memory_commitment_jobs`/`commitments` all reference `sessions(id)` (directly or transitively), `memory_proposals` references `memory_capture_jobs(id)`, and `commitments` also references `memory_commitment_jobs(id)` via `source_job_id`. This is a structural backstop for `delete_session()`'s existing manual same-order cleanup (MEM-GAP-003) — a session's messages/jobs/proposals/commitments now disappear even if application code ever fails to delete them explicitly first, or a raw `DELETE FROM sessions` bypasses the ORM-style methods entirely.
