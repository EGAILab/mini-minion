...
E2E encryption is intentionally not implemented — see
``docs/adr/0005-no-matrix-e2e-encryption.md``. This channel talks to a
self-hosted homeserver where the operator and the sole user are the same
person, so E2E's core threat model (an untrusted server operator reading
message content) doesn't apply; transport is already TLS-protected
regardless. Getting E2E working reliably also turned out to require either a
different Matrix library (matrix-nio has no cross-signing support) or a
from-scratch implementation of Element's modern verification protocol —
disproportionate cost for a threat this deployment doesn't have.
