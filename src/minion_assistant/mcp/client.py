...
--------------------------------
minion-assistant's Tool.execute() contract is synchronous (plain Python function,
...
# The try/except allows the rest of minion-assistant to import cleanly even if
...
class McpClientManager:
    """Manages all MCP server connections for one minion-assistant process.

    Lifecycle:
        manager = McpClientManager(server_configs, output_dir=Path("~/.minion-assistant/playwright-output"))
    ...
    def __init__(
        ...
        # thread exits, preventing minion-assistant from hanging at shutdown.
    ...
    def _run_sync(self, coro: Any, timeout: float = 60.0) -> Any:
        ...
        Why timeout?
        - A slow or broken MCP server should not hang minion-assistant indefinitely.
    ...
    def reconnect_all_sync(self, timeout: float = 120.0) -> None:
        ...
        restarting the minion-assistant process.
...
def _handle_image_content(block: Any, output_dir: Path | None) -> str:
    ...
    if output_dir is not None and img_bytes:
        ...
        try:
            ...
            # contract that the rest of minion-assistant's tool infrastructure relies on.
