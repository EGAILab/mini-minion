"""MCP (Model Context Protocol) client package for minion-assistant.

What is MCP?
------------
MCP is an open standard for connecting AI agents to external "tool servers".
Instead of hard-coding every tool into minion-assistant, you can run a separate
MCP server process (e.g. a filesystem server, a database server, a web
scraper) and minion-assistant connects to it and uses its tools automatically.

Think of it like a plugin system: MCP servers expose tools with JSON Schema
definitions. minion-assistant discovers those tools at startup and registers them
...
-------------------------------
    from minion_assistant.mcp import McpClientManager
