...
This module allows users to extend minion-assistant with custom tools, registry
...
workspace or in the project's ``.minion-assistant/`` directory.
...
1. Any Python file in the directory gets imported, which makes it easy to
   ...
   contains a ``.minion-assistant/`` directory with malicious files).
...
- **Global** (applies to all projects): ``~/.minion-assistant/plugins.json``
- **Local** (applies only to the current project):
  ``.minion-assistant/plugins.json`` (relative to the working directory)
...
Example ``plugins.json``::

    {
        "trust": "trusted",
        "tools": [
            "~/.minion-assistant/tools/my_grep_tool.py",
...
loader passes these paths to :func:`~minion_assistant.skills.discover_skills` and
...
Loaded commands are registered via :func:`~minion_assistant.commands.register_plugin_command`
...
def load_plugins(
    ...
    2. ``.minion-assistant/plugins.json`` — project-local plugins (cwd-relative).
    ...
    Args:
        ...
        workspace:       The user's workspace root (typically ``~/.minion-assistant``).
    ...
    manifest_paths: list[Path] = [
        ...
        Path.cwd() / ".minion-assistant" / "plugins.json",
    ]
...
def _load_manifest(
    ...
    min_version = raw.get("min_minion_assistant_version")
...
def _load_command_handler(name: str, description: str, module_path: Path) -> None:
    ...
    Registration is via :func:`~minion_assistant.commands.register_plugin_command`.
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            f"minion_assistant_cmd_{module_path.stem}",
            module_path,
        )
...
def _load_hook_module(registry: "ToolRegistry", module_path: Path) -> None:
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            f"minion_assistant_hook_{module_path.stem}",
            module_path,
        )
        if spec is None or spec.loader is None:
            return
...
def _load_tool_module(module_path: Path) -> list:
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            ...
            f"minion_assistant_plugin_{module_path.stem}",
            module_path,
        )
        if spec is None or spec.loader is None:
            return []
