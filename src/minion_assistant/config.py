...
calls — runs the moment any other module writes ``from minion_assistant.config import
agents``. This means:
...
# Resolve the project root: this file lives at src/minion_assistant/config.py,
...
def _validate(raw: dict) -> list[ConfigIssue]:
    ...
    # two fixed locations (~/.minion-assistant/plugins.json + .minion-assistant/plugins.json).
...
@dataclass(frozen=True)
class McpConfig:
    """All MCP servers configured for this minion-assistant instance."""
...
class StreamingConfig:
    ...
    - ``chat_mode``  — the interactive REPL (``uv run minion-assistant``).
...
def _resolve_mcp() -> McpConfig:
    ...
    for sname, sraw in servers_raw.items():
        ...
        #
        # Practical use cases:
        #   --output-dir ${USERPROFILE}\.minion-assistant\playwright-output  (Windows)
        #   --output-dir ${HOME}/.minion-assistant/playwright-output         (Linux/Mac)
        #   --storage-state ${HOME}/.minion-assistant/playwright-session.json
        #
...
# expanduser() converts "~/.minion-assistant" to the actual home-directory path.
workspace: Path = Path(_raw.get("workspace", {}).get("path", "~/.minion-assistant")).expanduser()
...
# fixed locations (~/.minion-assistant/plugins.json and .minion-assistant/plugins.json).
