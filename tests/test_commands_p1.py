...
from mini_minion.commands import CommandContext, CommandResult, dispatch_command
...
from mini_minion.tools import ToolRegistry, default_registry
...
def test_audit_shows_denied_entries(tmp_path):
    ...
    policy.audit_log.record(
        __import__("mini_minion.tools.audit", fromlist=["AuditEntry"]).AuditEntry(
...
def test_audit_respects_count_argument(tmp_path):
    from mini_minion.tools.audit import AuditEntry, _utcnow
